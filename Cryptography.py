...
class Salt:
    def __init__(self):
        self.salt: bytes = b""
        self.salt_filepath: str = self.get_salt_filepath()
    ...
    def generate_salt(self) -> bytes:
        """Generates a salt and writes it to salt file."""
    ...
    def get_salt_filepath(self) -> str:
        if Config.salt_filepath == "":
            self.salt_filepath = askopenfilename(title="Select salt or create a new one:", filetypes=[("Log files", "*.log")])
            Config.salt_filepath = self.salt_filepath
            if not self.is_salt_found():
                raise Exception("Selected salt not found.")
            if self.is_salt_empty():
                self.generate_salt()
        return Config.database_filepath

    def is_salt_found(self) -> bool:
        if exists(self.salt_filepath):
            self.is_salt_found_var = True
            return True
        else:
            return False

    def is_salt_empty(self) -> bool:
        return stat(self.salt_filepath).st_size == 0

    def read_salt_from_file(self) -> bytes:
        with open(self.salt_filepath, "rb") as salt_file:
            self.salt = salt_file.read()
            if len(self.salt) == 0:
                print(f"E: The salt file at {Config.salt_filepath} is empty.")
            Config.salt = self.salt
        return self.salt

    def write_salt_to_file(self):
        with open(self.salt_filepath, "wb") as salt_file:
