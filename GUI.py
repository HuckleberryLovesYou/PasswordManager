...
def password_gui():
    def call_convert_to_key():
        Cryptography.convert_master_password_to_key(user_master_password=master_password_entry.get())
        if PasswordManager.is_file_empty(Config.database_filepath) or Cryptography.decrypt_database(Config.database_filepath, Config.key):
            home()
        else:
            password_gui()
    ...
    def check_for_generate():
        def call_add(generate: bool = False):
            if generate:
                ...
                index, password = PasswordManager.add(title_entry.get(), username_entry.get(), password=PasswordManager.generate_password(password_length))
